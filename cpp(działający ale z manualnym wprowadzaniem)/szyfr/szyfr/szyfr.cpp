...
string encrypt(const string& input, const string& key_input) {
    ...
    for (size_t i = 0; i < input.length(); i++) {
        ...
        if (isalpha(letter)) {
            ...
            char e_letter = (letter - z + keys + 26) % 26 + z;
            output += e_letter;
        }
        else {
            output += letter;
...
string decrypt(const string& input, const string& key_input) {
    string output = "";
    int key_length = key_input.length();
    for (size_t i = 0; i < input.length(); i++) {
        char letter = input[i];
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "-d") == 0) {
            string input;
            string key_input;
            cout << "Wprowadź tekst do odszyfrowania: " << endl;
            getline(cin, input);
            cout << "Wprowadź klucz: " << endl;
            getline(cin, key_input);
            string decrypted = decrypt(input, key_input);
            cout << decrypted << endl;
        }
    }
}